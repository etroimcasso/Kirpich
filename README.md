...
- **Statistics, if you want them.** Switched on from the settings, the title screen grows a fourth
  item and the game starts keeping a record of itself: rounds played, time at the well, pieces
  dropped, score, lines and how they were cleared, each of the seven shapes, and the longest single
  round — all of it per difficulty combination and kept across launches. Five branches read it back:
  the whole game's totals, and one for each mode with a picker that moves the figures under it.
