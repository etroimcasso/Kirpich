...
| Type-C gameplay (the rising floor) | ✅ | [`features/type-c-rising-floor.md`](features/type-c-rising-floor.md) — a third game type: a count of drops on the mode's own four-readout backdrop under a `RISE` label, and when it empties the stack shifts up a row and a fresh garbage row arrives at the bottom. Picked as a level **and** a rise, six values from 16 down to 6, each pair keeping its own top scores |
...
| Settings screen | ✅ | [`features/settings.md`](features/settings.md) — fullscreen, window size, eighty colour palettes, the ghost piece, the extra game types, the statistics, a guarded quit, and a page of four guarded resets — the high scores, the statistics, the achievements, or all three, each clearing its own records and documents and leaving the others alone; reached from the title screen and from a paused round, with the choices saved beside the top scores |
...
| Statistics | ✅ | [`features/statistics.md`](features/statistics.md) — what has been played, kept per difficulty combination and across launches: rounds, time, drops, score, lines and their kinds, the seven shapes, and the longest single round with the combination it was played at. Always recorded, whether or not it is shown; the attract demo is left out, and quitting mid-round still records the round. Switched on from a settings row, it is reached from the title screen: five rows open paged readouts of the whole game's figures and of each mode's, with a picker that moves the figures under it. Once a heart game has been played, the All-Time branch gains an `all / normal / heart` filter and every level selector reaches the heart levels, with a heart on any combined-view record sourced from a single heart round. The achievements row opens their own screen and carries how many have been earned |
| Achievements | ✅ | [`features/achievements.md`](features/achievements.md) — thirty-six things worth doing across nine themed sections, awarded when a round ends and kept across launches, with no points and no completion score. Every condition reads a finished round's own state, the bonus scenes it reached, or the lifetime totals the statistics already keep, so the simulation is untouched and the attract demo earns nothing by construction. Only the two launch scenes are hidden until earned; the rest is a ladder and says what it asks for. Shown on the port's first declarative-component screens: one section to a page, a grid of badges with a corner selector, and a panel that opens on a badge with its criterion and the date it was earned. A round that earns something announces it on the way out — one to a screen, a press for the next — whether the round ended at the game-over screen or flew a rocket |
