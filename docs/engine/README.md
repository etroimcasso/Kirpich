...
| [settings.md](settings.md) | The player's display choices — the `Settings` value and its save document, the two screens that edit them, the twelve colour ramps the game can be drawn in and how a ramp reaches every palette, and the arrows and palette preview that are drawn as shapes rather than as cells. Covers adding a setting, a row, or a ramp. |
