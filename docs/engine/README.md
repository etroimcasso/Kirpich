...
| [new-mode.md](new-mode.md) | The six piece shapes the Game Boy game does not have — the shapes and their compile-time-derived orientations, the identity range that keeps the two sets apart, the cell and renderer forks that carry a three- or five-cell piece through machinery built for four, the block art and the tile indices it occupies, the wider draw, and the latch that decides which set a round is played with; where they live, and what to edit to change a shape, add one, or change when a round is New. |
