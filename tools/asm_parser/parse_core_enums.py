...
def _banner(source_commit: str) -> str:
    return common.banner("parse_core_enums.py", source_commit)
...
def emit_fixture(
    ...
    gs_rows = "\n".join(
        f'    {{0x{value:02X}, "{label}", "{common.cpp_escape(comment)}"}},'
        for value, label, comment in game_states
    )
    ss_rows = "\n".join(
        f'    {{0x{index:02X}, "{common.cpp_escape(label)}"}},' for index, label in serial_states
    )
    return f"""#pragma once
{_banner(source_commit)}
