...
// Type C - followed by the application total. Reusing that order rather than minting a second
// convention means one answer to "which block comes first" across both documents.
//
// A slice is its ten counts as little-endian 32-bit values, in the order StatSlice declares them.
// Every count is 32 bits wide whether or not it needs to be: a uniform slice makes the image's size a
// multiplication rather than a sum, and no count can ever be the one that overflows first.
//
// Only the tables and the application total are written. The round in progress is the current
// round's bookkeeping and means nothing after the program stops, so it is left out exactly as
// HighScoreState's name-entry bytes are left out of theirs.
...
//
// Version 1 is the first. When a later version appends a count or an axis, it registers its own
// migration chain in loadStats immediately before its own read - the store's version and its
// migrations are the store's, not the document's, and this store also carries the settings and the
// top scores at versions of their own.
...
inline constexpr std::size_t kStatSliceBytes = 40;  // ten 32-bit counts
...
inline constexpr std::size_t kStatsImageBytes =
    kStatsTypeBBytes + kStatsTypeABytes + kStatsTypeCBytes + kStatsApplicationBytes;
