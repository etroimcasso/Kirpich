...
// on, - for the two types that are picked with a second value - the start height or the rise index,
// and whether heart mode was on. Type A is picked by level alone, so it carries no second value.
//
// One derivation, shared by everything that keys a finished round. The top-score tables and the
// statistics tables both slice by this key, and two separate readings of the same flow state could
// come to disagree about which slice a round belongs in. Heart is part of the key rather than a level
// axis: a heart round is the same difficulty combination played harder, and it fills its own tables
// (src/state/stats_state.h, src/state/high_score_state.h) so a normal round's figures are never mixed
// with a heart one's.
struct RoundCombination {
    ...
    bool     heart      = false;  // heart mode was on for this round
...
[[nodiscard]] constexpr RoundCombination combinationOf(const GameFlowState& flow) noexcept {
    // heartMode stores the raw joypad byte, so any non-zero value means the mode is on (see the field
    // note above). This is the single site both the statistics and the top-score tables key on, so
    // adding heart here is what routes a heart round into its own tables.
    const bool heart = flow.heartMode != 0;
