...
// list, switch back to the first map, and hand off to the difficulty screen for the mode just played.
// Runs on its first frame: unlike every other handler here, it has no timer gate. `roundExit` decides
// where it actually leads - a rocket round leaves through this scene rather than the game-over screen,
// so this is where its achievement check and any notice run.
...
// after this runs. `roundExit` carries the rocket scene's exit, the point a rocket round truly ends;
// unset, that exit goes straight to the difficulty screen.
